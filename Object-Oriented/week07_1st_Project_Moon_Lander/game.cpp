...
void Game :: draw(const Interface & ui)
{
   ...
   lander.draw();
   drawText(Point(0.0, 185.0), "Directions: Use the Arrow Keys to");
   drawText(Point(0.0, 170.0), "maneuver the lander to the platform.");

   if (lander.isLanded())
   {
      drawText(Point(0.0, 150.0), "You have successfully landed!");
   }
   
   else if (!lander.isAlive())
   {
      drawText(Point(0.0, 150.0), "You have crashed!");
   }
   
   if (lander.getCanThrust())
   {
      drawLanderFlames(lander.getPoint(), ui.isDown(), ui.isLeft(), ui.isRight());
   }
   if (lander.getFuel() == 0)
   {
      drawText(Point(0.0, 135.0), "You are out of fuel!");
   }
   Point fuelLocation;
